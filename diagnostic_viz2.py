...
# import script that's calling this script... is that right?
#from schema_high_level_func import hs, a, choose_method, run_method
import schema_high_level_func as schema 

def wave_viz(make_heat=True, make_ts=True):
...
# Next we want to output a 17x30 array of max heights for each 6 hour time period (). There are 30 areas. 
    ...
    frame = schema.run_method(schema.m, schema.a, schema.hs)
    ...
    if make_heat==True:
        with sns.axes_style("whitegrid"):
            ...
            ax1.set_title("{} Significant Wave Heights".format(schema.chosen_stat), size=14)
    ...
    if make_ts==True:
        with sns.axes_style("darkgrid"):
             ...
             for area in frame.columns:
                 ...
                 color_num += 1
             ax2.set_title("{} Wave Heights".format(schema.chosen_stat), fontsize=14)
